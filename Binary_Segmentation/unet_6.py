...
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        enc2 = self.encoder2(self.dp(self.pool1(enc1)))
        enc3 = self.encoder3(self.dp(self.pool2(enc2)))
        enc4 = self.encoder4(self.dp(self.pool3(enc3)))
        enc5 = self.encoder5(self.dp(self.pool4(enc4)))
        enc6 = self.encoder6(self.dp(self.pool5(enc5)))
        bottleneck = self.bottleneck(self.dp(self.pool6(enc6)))
        dec6 = self.upconv6(bottleneck)
        dec6 = torch.cat((dec6, enc6), dim=1)
        dec6 = self.dp(self.decoder6(dec6))
        dec5 = self.upconv5(dec6)
        dec5 = torch.cat((dec5, enc5), dim=1)
        dec5 = self.dp(self.decoder5(dec5))
        dec4 = self.upconv4(dec5)
        dec4 = torch.cat((dec4, enc4), dim=1)
        dec4 = self.dp(self.decoder4(dec4))
        dec3 = self.upconv3(dec4)
        dec3 = torch.cat((dec3, enc3), dim=1)
        dec3 = self.dp(self.decoder3(dec3))
        dec2 = self.upconv2(dec3)
        dec2 = torch.cat((dec2, enc2), dim=1)
        dec2 = self.dp(self.decoder2(dec2))
